import React, { use, useEffect, useRef, useState, useEffect as useEffectHook, useMemo } from 'react'
...
const AdminUsers = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                ...
                console.error("Failed to fetch users. Please try again.", err);
                setLoading(false);
            }
        };
        fetchUsers();
    ...
    if (loading) {
        return <div className="loader-full-page">
                ...
                <div className={s.filterWrapper} ref={filterRef}>
                    ...
                    {openFilter && (
                        <div className={s.filterDropdown}>
                            <button onClick={() => { setRoleFilter("all"); setOpenFilter(false); }}
                                className={s.filterOption(roleFilter === "all")}
                            >
                                All Users
                            </button>
                            <button onClick={() => { setRoleFilter("buyer"); setOpenFilter(false); }}
                                className={s.filterOption(roleFilter === "buyer")}
                            >
                                Buyer
                            </button>
                            <button onClick={() => { setRoleFilter("seller"); setOpenFilter(false); }}
                                className={s.filterOption(roleFilter === "seller")}
                            >
                                Seller
                            </button>
                            <button onClick={() => { setRoleFilter("admin"); setOpenFilter(false); }}
            ...
            <div className={s.cardContainer}>
                <div className={s.cardHeader}>
                    <div className={s.cardTitleRow}>
                        <h1 className={s.cardTitle}>Platform Users</h1>
                        <div className={s.userCount}>
                            Showing{" "}
                            <span className={s.userCountSpan}>
                                ...
                                filteredUsers.map((user) => (
                                    <tr key={user._id} className={s.tableRow}>
                                            ...
                                            <div className='flex items-center gap-4'>
                                                ...
                                                <div>
                                                    ...
                                                    <div className={s.userInfoId}>
                                                        ID: {user._id.slice(-8).toUpperCase()}</div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td className={s.tdActions}>
                                            ...
                                            >
                                                {user.isBlocked ?(
                                                    <HiOutlineLockOpen size={18} />):(<HiOutlineLockClosed size={18} />
                                                    )}
                                            </button>
                                            <button
